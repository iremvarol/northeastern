...
if ($ && $.ui && $.ui.dialog) {
    $.extend($.ui.dialog.prototype.options, {
        open: function () {
            var e = $(document).find("html");
            e.css("overflow", "hidden");
            var f = e.find("body").scrollTop();
            if (f > 0) {
                e.scrollTop(0);
                var d = $(this);
                d.data("cacheScrollTop", f)
            }
            var a = $(this).closest(".ui-dialog");
            ...
            a.find(".ui-dialog-titlebar-close").attr("aria-label", "Close")
        },
        beforeClose: function () {
            var b = $(document).find("html");
            b.css("overflow", "");
            var c = $(this).data("cacheScrollTop");
            if (c) {
                b.find("body").scrollTop(c);
...
dnn.extend(dnn, {
    ...
    doDelay: function (b, c, d, a) {
        if (this.delay[b] == null) {
            ...
            this.delay[b].num = window.setTimeout(dnn.createDelegate(this.delay[b], this.delay[b].complete), c)
        }
    },
    ...
    encodeJSON: function (a) {
        return a.toString().replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/'/g, "\u0027").replace(/\"/g, "&quot;").replace(/\\/g, "\\\\")
    },
    evalJSON: function (a) {
        return Sys.Serialization.JavaScriptSerializer.deserialize(a)
    },
    escapeForEval: function (a) {
        return a.replace(/\\/g, "\\\\").replace(/\'/g, "\\'").replace(/\r/g, "").replace(/\n/g, "\\n").replace(/\./, "\\.")
    },
    ...
    addIframeMask: function (c) {
        if (dnn.dom.browser.isType("ie") && (c.previousSibling == null || c.previousSibling.nodeName.toLowerCase() != "iframe")) {
    ...
    removeIframeMask: function (a) {
        if (dnn.dom.browser.isType("ie") && (a.previousSibling != null && a.previousSibling.nodeName.toLowerCase() == "iframe")) {
...
dnn.extend(dnn.dom, {
    ...
    attachEvent: function (a, c, d) {
        if (a.addEventListener) {
            ...
            a.attachEvent(c, function () {
                dnn.dom.event = new dnn.dom.eventObject(window.event, window.event.srcElement);
                return d()
            })
        }
        return true
    ...
    loadScripts: function (a, b, c) {
        if (dnn.scripts == null) {
            ...
            dnn.dom.loadScript(dnn.dom.getScriptPath() + "dnn.scripts.js", null, e(a, b, c));
            return
        }
    ...
    setCookie: function (b, f, h, e, d, c, a) {
        ...
        document.cookie = b + "=" + escape(f) + ((g) ? "; expires=" + g.toGMTString() : "") + ((e) ? "; path=" + e : "") + ((d) ? "; domain=" + d : "") + ((c) ? "; secure" : "");
    ...
    getElementPostString: function (a) {
        ...
        if (c == "input") {
            ...
            if (d == "text" || d == "password" || d == "hidden" || ((d == "checkbox" || d == "radio") && a.checked)) {
